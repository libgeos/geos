...
std::vector<size_t> DBSCANClusterFinder::process(const std::vector<std::unique_ptr<geom::Geometry>> & components,
    ...
    for (size_t p = 0; p < components.size(); p++) {
        hits.clear();
        neighbors.clear();
        const geom::Geometry *gp = components[p].get();
        ...
        for (size_t q : hits) {
            ...
            if (p == q) {
                ...
                if (!prep) {
                    prep = geom::prep::PreparedGeometryFactory::prepare(components[p].get());
                }

                dist = components[p]->distance(components[q].get());
            }
