...
template<>
Envelope
Grid<bounded_extent>::grid_cell(size_t row, size_t col) const
{
    ...
    return {
        ...
        row == (rows() - 1) ? ymin() : (ymax() - static_cast<double>(row + 1) * dy()),
        col == (cols() - 1) ? xmax() : (xmin() + static_cast<double>(col + 1) * dx()),
...
template<>
Envelope
Grid<infinite_extent>::grid_cell(size_t row, size_t col) const
{
...
{
    return { grid.extent(), grid.dx(), grid.dy() , domain };
}
