...
BoundaryChainNoder::substring(const SegmentString* segString, std::size_t start, std::size_t end)
{
    ...
    return std::make_unique<BasicSegmentString>(std::move(pts), segString->getData());
}
    ...
    bool constructM)
{
    ...
    return std::make_unique<NodedSegmentString>(std::move(pts), constructZ, constructM, segString->getData());
}
