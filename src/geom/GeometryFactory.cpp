...
std::unique_ptr<LinearRing>
GeometryFactory::createLinearRing(std::shared_ptr<const CoordinateSequence> newCoords) const
{
    // Can't use make_unique with protected constructor
...
std::unique_ptr<CircularString>
GeometryFactory::createCircularString(std::shared_ptr<const CoordinateSequence> newCoords)
const
{
    if (!newCoords)
        return createCircularString(false, false);
