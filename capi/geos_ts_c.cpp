...
InputGeometry convertToLineIfNeeded(GEOSContextHandle_t extHandle, const Geometry* g) {
    if (extHandle->curveToLineParams.has_value() && g != nullptr && (isCurvedType(g->getGeometryTypeId()) || g->hasCurvedComponents())) {
        auto linearized = g->getLinearized(extHandle->curveToLineParams.value());
        linearized->setSRID(g->getSRID());
        return InputGeometry(std::move(linearized));
    }
...
//------------------------------------------------------------------
    ...
    GEOSBuildArea_r(GEOSContextHandle_t extHandle, const Geometry* g)
    {
        ...
        return convertCurvesAndExecute(extHandle, g, [](const Geometry* inputGeom) {
            BuildArea builder;
            auto out = builder.build(inputGeom);
            return out;
        });
    }
 ...
 */
    ...
    GEOSPolygonizer_getCutEdges_r(GEOSContextHandle_t extHandle, const Geometry* const* g, unsigned int ngeoms)
    {
        ...
        return execute(extHandle, [&]() {
            ...
            for(std::size_t i = 0; i < lines.size(); i++) {
                if (handle->lineToCurveParams && !linearized.empty()) {
                    linevec[i] = lines[i]->getCurved(handle->lineToCurveParams.value());
                } else {
                          ...
                          Geometry** cuts, Geometry** dangles, Geometry** invalid)
    {
        ...
        return execute(extHandle, [&]() {
            ...
            for(std::size_t i = 0; i < inputGeom->getNumGeometries(); ++i) {
                plgnzr.add(inputGeom->getGeometryN(i));
            }

            const GeometryFactory* gf = g->getFactory();
            const bool lineToCurve = extHandle->lineToCurveParams.has_value() && inputGeom.isLinearized();

            if(cuts) {
                std::vector<std::unique_ptr<Geometry>> linevec;
                for(const auto& line : plgnzr.getCutEdges()) {
                    linevec.push_back(lineToCurve ? line->getCurved(extHandle->lineToCurveParams.value()) : line->clone());
                }
            ...
            if(dangles) {
                std::vector<std::unique_ptr<Geometry>> linevec;
                for(const auto& line : plgnzr.getDangles()) {
                    linevec.push_back(lineToCurve ? line->getCurved(extHandle->lineToCurveParams.value()) : line->clone());
                }
            ...
            if(invalid) {
                std::vector<std::unique_ptr<Geometry>> linevec;
                for(const auto& line : plgnzr.getInvalidRingLines()) {
                    linevec.push_back(lineToCurve ? line->getCurved(extHandle->lineToCurveParams.value()) : line->clone());
                }
            ...
            if (lineToCurve) {
                out = out->getCurved(extHandle->lineToCurveParams.value());
            }
    ...
    GEOSLineMerge_r(GEOSContextHandle_t extHandle, const Geometry* g)
    {
        ...
        return convertCurvesAndExecute(extHandle, g, [extHandle](const Geometry* inputGeom) {
            GEOSContextHandleInternal_t* handle = reinterpret_cast<GEOSContextHandleInternal_t*>(extHandle);
            const GeometryFactory* gf = handle->geomFactory;
            LineMerger lmrgr;
            lmrgr.add(inputGeom);
    ...
    GEOSLineSubstring_r(GEOSContextHandle_t extHandle, const Geometry* g, double start_fraction, double end_fraction)
    {
        ...
        return convertCurvesAndExecute(extHandle, g, [start_fraction, end_fraction](const Geometry* inputGeom) {
            ...
            LengthIndexedLine lil(inputGeom);

            auto length = inputGeom->getLength();
