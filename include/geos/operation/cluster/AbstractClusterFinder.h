...
public:
     ...
     */
    std::vector<std::unique_ptr<geom::Geometry>> cluster(std::unique_ptr<geom::Geometry> && g);
     ...
     * @return a Geometry vector, with each entry representing a single cluster.
     */
    std::vector<std::unique_ptr<geom::Geometry>> cluster(const geom::Geometry& g);
...
protected:
     ...
     */
    virtual std::vector<size_t> process(const std::vector<std::unique_ptr<geom::Geometry>> & components,
                                        index::strtree::TemplateSTRtree<std::size_t> & index,
                                        UnionFind & uf);

    template<typename T>
    static size_t indexOf(const std::vector<T> & vec, void* item) {
        return static_cast<size_t>(static_cast<T*>(item) - &(vec[0]));
