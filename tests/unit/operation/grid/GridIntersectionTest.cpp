...
()
{
    ...
    auto rci2 = GridIntersection::grid_intersection(ex, *g->getEnvelopeInternal());
...
()
{
    ...
    ensure_THROW( GridIntersection::grid_intersection(extent, *g), std::exception);
}
...
() {
    ...
    auto rci_rect =  GridIntersection::grid_intersection(ex, *g->getEnvelopeInternal());
...
template<> template<> void object::test<38>() {
    set_test_name("subdividing a polygon that contains the grid intersects grid");
