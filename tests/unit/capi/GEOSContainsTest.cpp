...
void object::test<6>()
{
    set_test_name("GEOSContains with automatic linearization");
    useContext();
    ...
    ensure_equals(GEOSContains_r(ctxt_, geom1_, geom2_), 2);
    useCurveConversion();
    ensure_equals(GEOSContains_r(ctxt_, geom1_, geom2_), 1);
...
void object::test<10>()
{
    set_test_name("Empty MultiPoint is contained by MultiSurface (vacuous)");
...
void object::test<11>()
{
    set_test_name("Empty Point is contained by CurvePolygon (vacuous)");
