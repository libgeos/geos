...
struct test_concavehull_data {
    ...
    void
    checkHullByArea(const std::string& wkt, double threshold, const std::string& wktExpected)
    {
        std::unique_ptr<Geometry> geom = reader_.read(wkt);
        std::unique_ptr<Geometry> actual = ConcaveHull::concaveHullByArea(geom.get(), threshold);
        std::unique_ptr<Geometry> expected = reader_.read(wktExpected);
        ensure_equals_geometry(expected.get(), actual.get());
    }
...
void object::test<15>()
{
    checkHullByArea("MULTIPOINT ((10 10), (90 10), (30 70), (70 70), (50 60))",
        0.5, "POLYGON ((30 70, 70 70, 90 10, 50 60, 10 10, 30 70))" );
}
